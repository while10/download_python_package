...
### 作用
读取 `requirements.txt`，针对多个目标平台批量下载 Python 3.12 的 wheel / sdist 文件。流程分两阶段：
...
| 变量 | 位置 | 默认值 | 说明 |
|------|------|--------|------|
| `PACKAGE_LIST` | 第 18 行 | `.\requirements.txt` | 输入的包清单文件路径 |
| `DOWNLOAD_DIR` | 第 19 行 | `E:\python包下载\python312-packages` | **下载根目录**（由 `PYTHON_VERSION` 动态拼接），所有平台子目录、缓存、日志、进度文件均在此目录下 |
| `INDEX_URLS` | 第 24-27 行 | 阿里云 + 清华 | 镜像源（主源 + 备源），下载失败会自动切换 |
| `PLATFORMS` | 第 30-34 行 | `win_amd64`、`manylinux2014_x86_64`、`manylinux2014_aarch64` | 目标平台列表，删减可只下部分平台 |
| `GROUP_SIZE` | 第 48 行 | `100` | 每组包数量，影响分组数与单次解析规模 |
| `UV_COMPILE_TIMEOUT` | 第 51 行 | `600` | uv 依赖解析超时（秒） |
| `UV_DOWNLOAD_TIMEOUT` | 第 52 行 | `1800` | uv 下载超时（秒） |
| `PYTHON_VERSION` | 第 16 行 | `"3.12"` | 目标 Python 版本，修改此常量即可全局切换（含 `--python-version`、Lock 缓存校验、下载目录名） |
...
| 日期 | 改动 |
|------|------|
| 2026-07-27 | 抽取 `PYTHON_VERSION` 常量统一引用（含 `--python-version`、Lock 缓存校验、下载目录名），以后如需下载其它 Python 3.XX 版本，只需改动第 16 行一个值即可 |
